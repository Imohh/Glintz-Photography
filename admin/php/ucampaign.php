...
$id=$_POST['id'];
$query="SELECT * FROM campaigns_one WHERE id='$id'";
...
$query="UPDATE campaigns_one SET ";
...
$queryrun=mysqli_query($db,$query);
if($queryrun){
    header("location:../?editcampaign=true#done");
}    
...
if(isset($_GET['del'])){
    $id=$_GET['del'];
    $query="DELETE FROM campaigns_one WHERE id='$id'";
    $queryrun=mysqli_query($db,$query);
if($queryrun){
    header("location:../?editcampaign=true#done");
}
...
if($pdone=="error"){
    header("location:../?editcampaign=true&msg=error");
}else{
$query="INSERT INTO campaigns_one (projectname,projectpic,projectlink) ";
